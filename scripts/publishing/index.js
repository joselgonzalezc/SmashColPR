...
async function callingPublisher(settings) {
  ...
  let anyBranch = "",
    tag = "";
  const anyBranchIndex = proc.argv.findIndex(function (argv) {
    return argv.includes("--np-any-branch=");
  });
  const tagIndex = proc.argv.findIndex(function (argv) {
    return argv.includes("--np-tag=");
  });
  if (anyBranchIndex > -1) {
    const anyBranchArg = proc.argv[anyBranchIndex].split("--np-any-branch=");
    anyBranch =
      anyBranchArg[1] == null || anyBranchArg[1] === "" ? "" : "--any-branch";
  }
  if (tagIndex > -1) {
    const tagArg = proc.argv[tagIndex].split("--np-tag=");
    tag =
      tagArg[1] == null || tagArg[1] === ""
        ? ""
        : proc.argv[tagIndex].replace("--np-tag=", "--tag=");
  }
...
async function generatingDocument(settings) {
  ...
  await execute(
    `doxdox './dist/**/*.js' --output ${settings.docsFileName} --ignore './dist/cjs/**/*.js' --package ${settings.packagePath} --layout markdown`
  );
}
